...
Sim.prototype.sendMessage = function () {
    ...
    if (c)if (c instanceof Array)for (f = c.length - 1; f >= 0; f--)e = c[f], e !== a && e.onMessage && e.onMessage.call(e, a, b); else c.onMessage && c.onMessage.call(c, a, b); else for (var f = d.entities.length - 1; f >= 0; f--) {
        var e = d.entities[f];
...
Sim.Facility.prototype.useFCFS = function (a, b) {
    if (this.maxqlen === 0 && !this.free || this.maxqlen > 0 && this.queue.size() >= this.maxqlen)b.msg = -1, b.deliverAt = b.entity.time(), b.entity.sim.queue.insert(b); else {
        b.duration = a;
        var c = b.entity.time();
        this.stats.enter(c);
        this.queue.push(b, c);
...
Sim.Buffer.prototype.progressGetQueue = function () {
    for (var a; a = this.getQueue.top();)if (a.cancelled)this.getQueue.shift(a.entity.time()); else if (a.amount <= this.available)this.getQueue.shift(a.entity.time()), this.available -= a.amount, a.deliverAt = a.entity.time(), a.entity.sim.queue.insert(a); else break
};
Sim.Buffer.prototype.progressPutQueue = function () {
    for (var a; a = this.putQueue.top();)if (a.cancelled)this.putQueue.shift(a.entity.time()); else if (a.amount + this.available <= this.capacity)this.putQueue.shift(a.entity.time()), this.available += a.amount, a.deliverAt = a.entity.time(), a.entity.sim.queue.insert(a); else break
};
...
Sim.Store.prototype.progressGetQueue = function () {
    for (var a; a = this.getQueue.top();)if (a.cancelled)this.getQueue.shift(a.entity.time()); else if (this.current() > 0) {
        ...
        if (c)this.getQueue.shift(a.entity.time()), this.available--, a.msg = d, a.deliverAt = a.entity.time(), a.entity.sim.queue.insert(a); else break
    } else break
};
Sim.Store.prototype.progressPutQueue = function () {
    for (var a; a = this.putQueue.top();)if (a.cancelled)this.putQueue.shift(a.entity.time()); else if (this.current() < this.capacity)this.putQueue.shift(a.entity.time()), this.available++, this.objects.push(a.obj), a.deliverAt = a.entity.time(), a.entity.sim.queue.insert(a); else break
};
...
Sim.PQueue.prototype.insert = function (a) {
    ...
    for (var c = this.data, d = c[b]; b > 0;) {
        ...
        if (this.greater(c[f], a))c[b] = c[f], b = f; else break
    }
    c[b] = d
};
